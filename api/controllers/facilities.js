...
//     if (gallery_img) {
//       const deleteNoneMatchingImage =
//         "DELETE FROM facility_img WHERE Fac_Id = ? AND FacImg_Name NOT IN (?)";
//       const deleteNoneMatchingImageValues = [facId, gallery_img];
//       db.query(
//         deleteNoneMatchingImage,
//         deleteNoneMatchingImageValues,
//         (err, data) => {
//           if (err) {
//             return res.status(500).json("Database err: " + err.message);
//           }
...
//         "INSERT INTO facility_images (`Fac_Id`,`FacImg_Name`,`Is_Featured` ) VALUES ?";
//       const insertNewGalleryValues = gallery_img.filter(
//         (filename) =>
//           !existingGalleryImgs
//             .includes(filename)
...
//   // Function to get existing gallery images for the facility
//   const getExistingGalleryImages = () => {
...
//           reject(err);
//         } else {
//           resolve(existingGalleryImages);
//         }
//       });
//     });
//   };

//   // Function to update facility data
...
//   // Function to handle updating featured image
//   const updateFeaturedImage = (existingGalleryImages) => {
//     if (featured_img) {
//       return new Promise((resolve, reject) => {
//         // Update the featured image in the facility_images table
//         const qUpdateFeaturedImage =
//           "UPDATE facility_images SET `FacImg_Name` = ?, `Is_Featured` = 1 WHERE `Fac_Id` = ? AND `Is_Featured` = 1";
//         const featuredValues = [featured_img, facId];

//         db.query(qUpdateFeaturedImage, featuredValues, (err, data) => {
//           if (err) {
//             reject(err);
//           } else {
//             resolve(existingGalleryImages);
...
//       });
//     } else {
//       resolve(existingGalleryImages); // No featured image to update
//     }
//   };

//   // Function to update gallery images
//   const updateGalleryImages = (existingGalleryImages) => {
//     return new Promise((resolve, reject) => {
//       if (gallery_img) {
...
//         db.query(deleteNonMatchingImage, [deleteNonMatchingImageValues], (err, data) => {
//           if (err) {
//             reject(err);
//           } else {
...
//               .filter((filename) => !existingGalleryImages.map((img) => img.FacImg_Name).includes(filename))
//               .map((filename) => [facId, filename, 0]);
...
//               });
//             } else {
//               resolve(); // Resolve when there are no new images to insert
//             }
//           }
//         });
//       } else {
//         resolve(existingGalleryImages); // No gallery images to update
//       }
//     });
//   };

//   // Execute the Promise chain
//   getExistingGalleryImages()
//     .then(updateFacilityData)
//     .then(updateFeaturedImage)
//     .then(updateGalleryImages)
//     .then(() => {
//       return res.json("Facility has been updated");
//     })
//     .catch((err) => {
...
//       db.query(existingGalleryImagesQuery, [facId], (error, existingGalleryImages) => {
//         if (error) {
//           return res.status(500).json("Database error: " + error.message);
//         }
