...
export const updateFeedback = async (req, res) => {
  ...
  if (!token) return res.status(401).json('Not Authenticated to Update this feedback')
   ...
   try {
    ...
    const [users] = await db.query("SELECT users.id, users.username, users.email, role.Role_Name FROM users INNER JOIN role ON users.role = role.Role_ID WHERE users.id = ?", [userId]);

    if (users.length === 0 || users[0].Role_Name !== 'Admin') {
      return res.status(401).json('Only admins are allowed to update this feedback')
    }

    const q = "UPDATE feedback SET `FeedBack_Status` = ?, `approved_By` = ? WHERE `FeedBack_ID` = ?"
